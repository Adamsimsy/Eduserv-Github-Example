...
scContentEditor.prototype.showOutOfFrameGallery = function(sender, evt, destination, dimensions, params, httpVerb) {
  ...
  if (sender && Object.prototype.toString.call(sender).toLowerCase() === "[object string]") {
...
scContentEditor.prototype.changeSearchCriteria = function(index, name, title) {
  var table = $("SearchOptionsList");
  
  var row = table.rows[parseInt(index, 10)];

  row.cells[0].childNodes[0].innerHTML = name + ":";
  row.cells[0].childNodes[0].setAttribute("title", title);
...
scContentEditor.prototype.editRichText = function (url, key, html) {
  ...
  if (w) {  
    ...
    Element.insert(window.top.document.body, '<div id="ModalOverlayRTE" style="position: absolute; left: 0px; top: 0px; width: 100%; height: 100%; z-index: 100; background-color: rgb(170, 170, 170); filter:alpha(opacity=50); opacity: 0.5;"></div>');
  }
  else {
    w = $('feRTEContainer');
